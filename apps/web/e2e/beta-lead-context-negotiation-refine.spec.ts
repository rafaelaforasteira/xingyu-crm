...
const SCREENSHOT_DIR = path.join(__dirname, ".beta-screenshots", "lead-context-negotiation-refine");

test.describe("Lead context negotiation refine", () => {
  test.setTimeout(240_000);

  test("shows Lead # / Pipeline / Etapa without count, title, or open link", async ({ page }) => {
    ...
    await expect(summary.getByTestId("lead-context-contact-name")).toContainText(/Luciana/i);
    ...
    // Open by default: header still has no numeric badge
    ...
    await expect(negotiation.getByTestId("lead-context-lead-code")).toContainText(/Lead #\d{4,}/);
    await expect(negotiation.getByTestId("lead-context-pipeline")).toContainText(/Pipeline:/i);
    await expect(negotiation.getByTestId("lead-context-stage")).toContainText(/Etapa:/i);

    // Order: Lead # before Pipeline before Etapa
    const leadBox = await negotiation.getByTestId("lead-context-lead-code").boundingBox();
    const pipelineBox = await negotiation.getByTestId("lead-context-pipeline").boundingBox();
    ...
    await expect(negotiation.getByText(/Lead sem resposta/i)).toHaveCount(0);
    await expect(panel.getByText(/Abrir negociação/i)).toHaveCount(0);
    await expect(panel.getByRole("link", { name: /abrir negociação/i })).toHaveCount(0);
    ...
    await expect(panel.getByRole("button", { name: /Outras negociações/i })).toBeVisible();
    ...
    await expect(negotiation.getByTestId("lead-context-stage")).toContainText(nextStage, {
      timeout: 15_000,
    });
    await expect(summary.getByTestId("lead-context-stage-badge")).toContainText(nextStage);
    ...
    await page
      ...
      .click();
    await expect(negotiation.getByTestId("lead-context-stage")).toContainText(originalStage, {
      timeout: 15_000,
    });
