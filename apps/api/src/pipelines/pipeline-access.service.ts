...
export class PipelineAccessService {
  ...
  async assertAccess(user: AuthenticatedUser, pipelineId: string) {
    ...
    if (ids !== null && !ids.includes(pipelineId)) throw new ForbiddenException("Sem acesso a este pipeline.");
  }
  ...
  async assertTaskAccess(user: AuthenticatedUser, taskId: string) {
    return this.assertResourceAccess(user, "task", taskId);
  }
  ...
  private async assertResourceAccess(user: AuthenticatedUser, resource: PipelineResource, id: string) {
    let pipelineId: string | null | undefined;
    if (resource === "deal") {
      pipelineId = (await this.prisma.deal.findFirst({ where: { id, organizationId: user.organizationId, deletedAt: null }, select: { pipelineId: true } }))?.pipelineId;
    } else if (resource === "conversation") {
      pipelineId = (await this.prisma.conversation.findFirst({ where: { id, organizationId: user.organizationId, deletedAt: null }, select: { deal: { select: { pipelineId: true } } } }))?.deal?.pipelineId;
    } else if (resource === "task") {
      const task = await this.prisma.task.findFirst({ where: { id, organizationId: user.organizationId, deletedAt: null }, select: { pipelineId: true, deal: { select: { pipelineId: true } } } });
      pipelineId = task?.deal?.pipelineId ?? task?.pipelineId;
    } else if (resource === "note") {
      pipelineId = (await this.prisma.note.findFirst({ where: { id, organizationId: user.organizationId, deletedAt: null }, select: { deal: { select: { pipelineId: true } } } }))?.deal?.pipelineId;
    } else if (resource === "activity") {
      const activity = await this.prisma.activity.findFirst({ where: { id, organizationId: user.organizationId }, select: { deal: { select: { pipelineId: true } }, task: { select: { pipelineId: true, deal: { select: { pipelineId: true } } } }, order: { select: { deal: { select: { pipelineId: true } } } } } });
      pipelineId = activity?.deal?.pipelineId ?? activity?.task?.deal?.pipelineId ?? activity?.task?.pipelineId ?? activity?.order?.deal?.pipelineId;
    } else {
      pipelineId = (await this.prisma.order.findFirst({ where: { id, organizationId: user.organizationId, deletedAt: null }, select: { deal: { select: { pipelineId: true } } } }))?.deal?.pipelineId;
    }
  ...
  async assertEligibleUser(user: AuthenticatedUser, pipelineId: string, candidateId?: string | null) {
  ...
  async overview(organizationId: string) {
    const [pipelines, teams, users] = await Promise.all([
      ...
      this.prisma.team.findMany({ where: { organizationId, deletedAt: null }, include: { members: { where: { deletedAt: null }, select: { id: true, name: true, avatarUrl: true } } }, orderBy: { name: "asc" } }),
      this.prisma.user.findMany({ where: { organizationId, deletedAt: null }, include: { team: { select: { id: true, name: true } } }, orderBy: { name: "asc" } }),
    ]);
    return { pipelines, teams, users };
  }

  async update(organizationId: string, pipelineId: string, input: { accessMode: "ORGANIZATION" | "RESTRICTED"; teamIds?: string[]; userIds?: string[] }) {
    const teamIds = [...new Set(input.teamIds ?? [])];
    const userIds = [...new Set(input.userIds ?? [])];
    if (input.accessMode === "RESTRICTED" && !teamIds.length && !userIds.length) throw new BadRequestException("Acesso restrito exige ao menos uma equipe ou pessoa.");
    const pipeline = await this.prisma.pipeline.findFirst({ where: { id: pipelineId, organizationId, deletedAt: null }, select: { id: true } });
    ...
    if (teamCount !== teamIds.length || userCount !== userIds.length) throw new BadRequestException("Equipe ou pessoa inválida para esta organização.");
    await this.prisma.$transaction(async (tx) => {
      ...
      for (const teamId of teamIds) await tx.$executeRaw`INSERT INTO "PipelineTeamAccess" (id,"organizationId","pipelineId","teamId") VALUES (${`pta-${pipelineId}-${teamId}`},${organizationId},${pipelineId},${teamId}) ON CONFLICT DO NOTHING`;
      for (const userId of userIds) await tx.$executeRaw`INSERT INTO "PipelineUserAccess" (id,"organizationId","pipelineId","userId") VALUES (${`pua-${pipelineId}-${userId}`},${organizationId},${pipelineId},${userId}) ON CONFLICT DO NOTHING`;
